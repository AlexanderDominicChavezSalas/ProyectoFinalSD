...
public class Bank_A  implements RemoteInterface  {
	...
	public void crearCuenta (String nombre) {
        ...
        {
			//
			ficheroList = new FileWriter("C:/Users/Asus/Documents/Compartida/ProyectoFinal/Bank_A/src/main/java/com/soap/ws/Archivos/ListaDeCuentas.txt",true);
			pwList = new PrintWriter(ficheroList);
			pwList.print(user.getNombre() + " | ");
			pwList.println(user.getN_cuenta());

            fichero = new FileWriter("C:/Users/Asus/Documents/Compartida/ProyectoFinal/Bank_A/src/main/java/com/soap/ws/Archivos/" + 
	...
	public void retiro(String nCuenta, float x)  throws RemoteException {
		...
		for(int i = 0 ; i <lista_usuarios.size(); i++){
			//Primero obtenemos al usuario de dicha cuenta
			if(lista_usuarios.get(i).getN_cuenta().equals(nCuenta)&&lista_usuarios.get(i).getSaldo()>x){
